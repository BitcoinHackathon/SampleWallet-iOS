...
class APIClient {
    ...
    func getTxDetail(withTxID txID: String, completionHandler: @escaping (CodableTx) -> ()) {
        let url = "\(apiEndPoint)/tx/\(txID)"
        print("get tx detail url = \(url)")
        get(url: url, completion: { (data) in
            do {
    ...
    func postTx(withRawTx rawTx: String, completionHandler: @escaping (_ txId: String?, _ error: String?) -> ()) {
        ...
        post(url: url, parameters: ["rawtx": rawTx], completion:  { (data) in
            ...
            do {
                print(data.hex)
                let tx = try JSONDecoder().decode(SentResponse.self, from: data)
